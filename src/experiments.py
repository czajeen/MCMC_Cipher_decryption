...
def plot_accuracy_histogram(r: dict) -> None:
    ...
    suffix = f"transp_k{r['key_length']}_{r['text_length']}" if is_trans else str(r["text_length"])
...
def plot_comparison_accuracy_vs_length(
    ...
    ax.plot(s_lengths, s_means, "bo-", linewidth=2, markersize=8, label="Podstawieniowy (26!)")
    ax.fill_between(s_lengths, s_ci_low, s_ci_high, alpha=0.15, color="blue")

    ax.plot(t_lengths, t_means, "gs-", linewidth=2, markersize=8, label=f"Transpozycja k={trans_results[0]['key_length']} ({trans_results[0]['key_length']}!)")
